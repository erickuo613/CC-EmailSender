...
直接使用該郵箱作為寄件人，**無需再次詢問**。
立即執行 SMTP 自動偵測（見 1a-偵測）。

**情況 B — 使用者未提供寄件人郵箱：**

使用 `AskUserQuestion` 提供一個「寄件人郵箱」輸入框，讓使用者輸入自定義郵箱地址：
...
```json
{
  "questions": [{
    ...
    "header": "寄件人郵箱",
    "options": [
      {"label": "Gmail", "description": "使用 Gmail 郵箱，請點擊下方 Other 輸入完整郵箱地址"},
      {"label": "網易/QQ", "description": "使用 163/126/QQ 等國內郵箱，請點擊下方 Other 輸入完整郵箱地址"},
      {"label": "Outlook", "description": "使用 Outlook/Hotmail 郵箱，請點擊下方 Other 輸入完整郵箱地址"}
    ]
  }]
}
```

使用者透過底部 **Other 自定義輸入框** 輸入完整郵箱地址後，繼續執行 SMTP 偵測。
